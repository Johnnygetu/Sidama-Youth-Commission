...
header('X-Endpoint-Name: news/updateNews');
...
try {
    ...
    if (empty($id)) throw new Exception('News ID is required');
    ...
    $existing = $db->fetchOne('SELECT id, image_url FROM news WHERE id = :id', ['id' => $id]);
